...
Convention métrique HISTORIQUE (héritée de refit_polish / mini-cover) :
  ...
  Ce n'est PAS le pullback chain-rule holomorphe standard (qui donnerait
  Wᵀ·gI au lieu de W†·gI) mais la convention contre laquelle le witness
  a été fitté ET certifié. Les deux formes sont hermitiennes valides ;
  seule la convention refit est cohérente avec ε̃ = 2.824e-4 et le
...
8 feuilles reconstruites, point gardé ssi CE (S, g) est le propriétaire
...
def sample_chart(rng, S, g_col, n_draw, uv_offset=(0., 0., 0., 0.)):
    """Tire n_draw (u,v) uniformes sur [-1,1]^4 dans le chart (S, g_col),
    ...
    ⟹ symétries Z_2 : u → -u, v → -v. Choisir un décalage NON symétrique
    (composantes non nulles et disparates) casse ces Z_2 dans le sample,
...
def detMS_on_block(b):
    """det M_S = 8 Z_i Z_j Z_k V_S sur le bloc (pour la densité de Omega)."""
    i, j, k = b["S"]
...
def multi_values_and_projected_grads(Z, W, multis):
    ...
    (convention refit_polish / mini-cover) :
        p[k, I, α] = Σ_a conj(W[k, a, α]) · (∂z^I/∂Z_a)(Z[k])
                   = (W†·∂z^I)[k, α]
    Cette convention n'est PAS le pullback standard (Wᵀ·∂z^I via règle de
    chaîne holomorphe pure), mais celle contre laquelle le witness a été
    fitté (`k3_cap_d2_kahler_refit_polish.py` : A = einsum('nab,nb->na',
    Vh, gI) avec Vh = conj(V.T)) et celle qui certifie la positivité
...
def sphere_horizontal_frame(Z, W):
    ...
    Le witness a été fitté sur la sphère unité (Z_sph tel que |Z|=1) avec
    V orthonormé + horizontal (SVD de [J_Q ; Z̄]) et correction −f·I_2.
    Ce format est LA convention où c_a a un sens.

    Dans le chart radical, on a (Z, W) avec |Z|²=s et jauge Z_g=1 ; W = ∂Z/∂w
    n'est ni horizontal ni orthonormé. La covariance de frame donne
    ...
    Propriétés vérifiées :
      ...
      U engendre le MÊME plan horizontal que V ⟹ ∃ A = V†U inversible tel
      que U = V·A  ⟹  g_chart = A† · g_V · A (covariance).

    La correction homogénéisée devient exactement −f·U†U (au lieu de −f·I_2
    qui suppose une frame orthonormée). Retourne (z, U)."""
...
def chart_metric(Z, W, M, coeffs, basis, multis, midx, want_element_data=False):
    ...
    sphere_horizontal_frame — le caller (dry_run) fait la conversion une fois.
    Formule ambiante (comme metric_interval_kahler / refit_polish) :
    bloc ρ + Σ c·T_raw − f·U†U. Positivité garantie (witness certifie
    4000/4000 sur cette frame par covariance U = V·A). Convention volume
...
def F0_pointwise(G, jjh, GFS):
    ...
    G_U et G_FS_U = U†U pullback dans une frame U = V·A non-orthonormée
    (chart radical horizontal). Sous changement V → U : det G_U = |det A|²·
    det G_V et det(U†U) = |det A|². Le facteur |Ω|²(U₁,U₂) = |det A|²·
    |Ω|²(V₁,V₂) est absorbé par la forme volumique dV_0 = det G_U·d^4(u,v).
    ⟹ r_0 est invariant de frame : r_U = r_V à la précision machine.
...
def jjh_on_sphere(z):
    ...
    Nécessite z sur la sphère (|z|²=1) pour matcher la convention refit."""
...
def basis_chart_derivs(basis, m, p, s, midx, Z, W):
    ...
    ⚠️ 2026-07-11 — DIAGNOSTIC UNIQUEMENT. Cette routine construit le
    Laplacien en FORME FORTE (∂∂̄q via chain rule + corrections) et n'est
    utilisée que pour le test d'auto-adjonction. Le pivot spectral core
    passe désormais par la forme faible (matrices M, K) — cf. GPT 07-11 :
    la partition owner crée des flux artificiels que la forme faible
    élimine.
    ...
    d·φ·s^{-(d+1)}·(W†W) sont écrites pour la convention ambiante refit."""
    K = m.shape[0]
    nb = len(basis)
