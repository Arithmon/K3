...
the paper, `docs/RESULTS_INDEX.md` names the generator, the artifact and
how the verification command checks it. Each generator is an ordinary Python
module, run in the pinned environment; the full chain takes hours, which is
why it is kept apart from the one-command check. The figures are regenerated by their own
